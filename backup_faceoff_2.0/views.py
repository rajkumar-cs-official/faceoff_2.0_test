...
#         super(AudioClassifier, self).__init__()
#         self.lstm = nn.LSTM(input_dim, hidden_dim, batch_first=True, bidirectional=True)
#         self.dropout1 = nn.Dropout(dropout_rate)  # After LSTM
...
#             nn.AdaptiveAvgPool1d(1),
#         )
#         self.dropout2 = nn.Dropout(dropout_rate)  # After CNN
#         self.fc = nn.Linear(64, num_classes)

#     def forward(self, x):
#         # x shape: [batch, seq_len, input_dim]
#         self.lstm.flatten_parameters()
#         x, _ = self.lstm(x)  # Output: [batch, seq_len, hidden_dim*2]
#         x = self.dropout1(x)

#         x = x.permute(0, 2, 1)  # Shape: [batch, hidden_dim*2, seq_len]
#         x = self.cnn(x).squeeze(-1)  # Shape: [batch, 64]
#         x = self.dropout2(x)

#         return self.fc(x)  # Shape: [batch, num_classes]
...
#         face_tensor = self.transform(Image.fromarray(face)).to(device)
#         lips, eyes = self.extract_lip_eye(face_tensor)
#         lips, eyes = lips.to(device), eyes.to(device)
#         local_feat = torch.cat([
#             self.local_cnn(lips),
#             self.local_cnn(eyes)
#         ], dim=1)
#         global_feat = self.vit(face_tensor.unsqueeze(0)).to(device)
#         return torch.cat([local_feat, global_feat], dim=1)
...
#         # Use the same names as in your saved state dict
#         self.classifier_bn_pool = torch.nn.Sequential(
#             torch.nn.BatchNorm1d(128),
#             torch.nn.AdaptiveAvgPool1d(1)
#         )
...
#         x = fused.transpose(1, 2)                 # (B, 128, T)
#         x = self.classifier_bn_pool(x).squeeze(-1) # (B, 128)
#         return torch.sigmoid(self.classifier_fc(x)).squeeze(-1)
    

# face_detector = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
            
# # Initialize facemark with proper error checking
# facemark = cv2.face.createFacemarkLBF()


# facemark.loadModel('/home/student/new_api/testpro-main/models/lbfmodel.yaml')
...
#         # Define model paths relative to settings
#         self.MODEL_PATHS = {
#             'audio': os.path.join(settings.BASE_DIR, 'models/best_model_epoch_5.pth'),
#             'visual_extractor': os.path.join(settings.BASE_DIR, 'models/visual_feature_extractor.pth'),
#             'classifier': os.path.join(settings.BASE_DIR, 'models/deepfake_classifier_frame.pth'),
#             'lbf': os.path.join(settings.BASE_DIR, 'models/lbfmodel.yaml'),
#         }
...
#             # Load Audio Model
#             self.audio_model = AudioClassifier().to(self.device)
#             state_dict = torch.load(self.MODEL_PATHS['audio'], map_location=self.device)
#             self.audio_model.load_state_dict(state_dict, strict=False)
#             self.audio_model.eval()

#             # Load Visual Feature Extractor
#             self.extractor = VisualFeatureExtractor().to(self.device)
#             state_dict = torch.load(self.MODEL_PATHS['visual_extractor'], map_location=self.device)
#             self.extractor.load_state_dict(state_dict)
#             self.extractor.eval()

#             # Load Deepfake Classifier
#             self.model = DeepfakeClassifier(896).to(self.device)
#             state_dict = torch.load(self.MODEL_PATHS['classifier'], map_location=self.device)
...
#     # --- STEP 2: Split Audio into 3-sec Chunks ---
#     def split_audio(self, audio_path, chunk_duration=3, target_sr=16000):
#         wav, sr = librosa.load(audio_path, sr=target_sr)
#         chunk_len = int(chunk_duration * target_sr)
#         chunks = [wav[i:i+chunk_len] for i in range(0, len(wav), chunk_len) if len(wav[i:i+chunk_len]) == chunk_len]
...
#     # --- STEP 3: Predict with Model on Each Chunk ---
#     def predict_chunks(self, chunks, processor, wav2vec2, model, device):
#         model.eval()
#         preds = []
...
#         fake_count = preds.count(1)  # Assuming 1 = fake
#         fake_ratio = fake_count / len(preds)

#         label = "Fake" if fake_ratio >= 0.2 else "Real"
#         print(f"🎬 Video classified as: *{label.upper()}* ({fake_count}/{len(preds)} fake frames - {fake_ratio*100:.1f}%)")
#         return label

#     def predict_from_video(self, video):
...
#     def extract_audio_features_from_video(self, video_path):
#         audio_path = self.extract_audio_from_video_v2(video_path)
#         waveform, sr = torchaudio.load(audio_path)
#         os.remove(audio_path)
#         if sr != self.hubert_sample_rate:
...
#         with torch.no_grad():
#             feats = self.hubert_model(waveform.to(device))[0][0].cpu().numpy()
...
#             tensor = self.visual_transform(crop).unsqueeze(0).to(device)
#             with torch.no_grad():
#                 feat = cnn_model.forward_features(tensor)
#                 pooled = torch.nn.functional.adaptive_avg_pool2d(feat, (1,1))
#                 lip_features.append(pooled.view(-1).cpu().numpy())
...
#     def predict(self, video_path, model_path="/home/student/new_api/testpro-main/models/sync_model_opencv_hubert_pytorch(3).pth", threshold=0.5):
#         print(f"Processing: {os.path.basename(video_path)}")
#         audio_segs = self.extract_audio_features_from_video(video_path)
#         visual_segs = self.extract_visual_features_from_video(video_path)
#         if not audio_segs or not visual_segs:
#             print("Could not extract features."); return None

#         model = SyncModel().to(device)
#         model.load_state_dict(torch.load(model_path, map_location=device))
#         model.eval()

#         scores = []
#         with torch.no_grad():
#             for a, v in zip(audio_segs, visual_segs):
#                 a_t = torch.tensor(a, dtype=torch.float32).unsqueeze(0).to(device)
#                 v_t = torch.tensor(v, dtype=torch.float32).unsqueeze(0).to(device)
#                 scores.append(model(a_t, v_t).item())
...
#         if label=="Real":
#             reason = (f"The average lip-audio misalignment score was low "
#                     f"({avg_score:.2f}), indicating synchronized speech.")
#         else:
#             reason = (f"The average lip-audio misalignment score was high "
#                     f"({avg_score:.2f}), indicating out-of-sync speech.")

#         print(f"Prediction: {label} | Confidence: {avg_score:.4f}")
#         print("Why:", reason)
#         return label, avg_score
...
#             print("\n--------------------audio check---------------------\n")
#             pred1 = self.classify_video(video_absolute_path, self.processor, self.wav2vec2, self.audio_model, device)
#             audio_p = self.label_to_prob(pred1)
#             print("\n--------------------audio check---------------------\n")

#             print("\n--------------------visual check---------------------\n")
#             pred2 = self.predict_from_video(video_absolute_path)
#             visual_p = self.label_to_prob(pred2)
#             print("Prediction:", pred2)
#             print("\n--------------------visual check---------------------\n")

#             print("\n--------------------lip sync check---------------------\n")
#             pred3, _ = self.predict(video_absolute_path)
#             sync_p = self.label_to_prob(pred3)
#             print("\n--------------------lip sync check---------------------\n")

#             print("\nFinal Prediction:")
#             # if pred1 == "Real" and pred2 == "Real" and pred3 == "Real":
#             #     print("The video is a real video.")
#             #     final_verdict = "Real"
#             # else:
#             #     print("The video is a deepfake video.")
#             #     final_verdict = "Fake"

#             combined_score = visual_p * 0.3 + audio_p * 0.2 + sync_p * 0.5
...
class Posture(View):
    ...
    def post(self, request):
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_filename = f"posture_video_{timestamp}.mp4"
